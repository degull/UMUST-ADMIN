...
const InfoForm = ({ InfoData, onEditComplete }) => {
   ...
   const readFileAsBlob = (file) => {
    ...
    const handleSubmit = async (event) => {
      ...
      try {
        ...
        const apiUrl = InfoData ? `https://umust302.shop/api/articles/${InfoData.id}` : 'https://umust302.shop/api/articles';
   ...
   return (
    <S.InfoFormContainer>
      <Main />
      <S.FormContainer>
        <S.FormTitle>제품 관련 정보 작성</S.FormTitle>

        <S.InfoForm onSubmit={handleSubmit}>
        ...
        <S.MarkdownPreviewContainer>
          <h2>Markdown 미리보기</h2>
          <S.InfoContent>{markdownContent}</S.InfoContent>
        </S.MarkdownPreviewContainer>
      </S.FormContainer>
    </S.InfoFormContainer>
   );
