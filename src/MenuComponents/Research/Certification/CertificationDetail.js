...
const CertificationDetail = ({ onDelete, onEdit }) => {
   ...
   useEffect(() => {
    ...
    const handleConfirmDelete = async () => {
      try {
        const deleteResponse = await axios.delete(`https://umust302.shop/api/articles/${certificationId}`);
    ...
    const handleEdit = () => {
      ...
      navigate(`/Research/certifications/${certificationId}/edit`);
    };
   ...
   return (
      <S.AlbumDetailContainer>
        ...
        <S.AlbumDetails>
            <span>작성자: {Certification.createdBy || '짱구'}</span>
