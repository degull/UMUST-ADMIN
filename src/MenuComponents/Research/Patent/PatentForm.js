...
const PatentForm = () => {
   ...
   const handleFileUpload = async (file) => {
    ...
    const handlePaste = (patent) => {
      const items = (patent.clipboardData || patent.originalEvent.clipboardData).items;
      for (let i = 0; i < items.length; i++) {
        if (items[i].type.indexOf('image') !== -1) {
          const blob = items[i].getAsFile();
          handleImageUpload([blob]);
          patent.preventDefault();
          break;
        }
      }
    };
   ...
   return (
      <S.AlbumFormContainer>
         <Main />
         <S.AlbumContainer>
            <S.FormTitle>특허 게시판 작성</S.FormTitle>
